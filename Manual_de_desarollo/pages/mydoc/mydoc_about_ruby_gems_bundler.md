...
TomTom es un conjunto de  servicio terciario que ofrece diferentes servicios de mapas y rutas pero con más alternativas o más personalizables de lo que posee Google cloud platform. Dice poder ser más escalable que la competencia. Para el proyecto se utiliza la api routing .

Dentro de API routing se desea implementar Calculate Route. 


Este servicio calcula una ruta entre un origen y un destino, pasando por waypoints (si se especifica). Se pueden tener en cuenta parámetros de enrutamiento adicionales como tráfico, condiciones de evitación, hora de salida / llegada, etc.

La API request tiene la siguiente estructura utilizada con el formato GET:

```
https://api.tomtom.com//routing/{versionNumber}/calculateRoute/{locations}/{contentType}
```
Locations tiene que venir con formato:

```
lat_init,lng_init:lat_final,lng_final
```

Content type por su lado posee múltiples opciones. Se puede configurar el formato de salida, número máximo de alternativas , routes representation, modo de viaje, tipo de ruta, tráfico entre otros. Para el proyecto se escoge la siguiente configuración:

```
contentTuype=json?maxAlternatives=5&routeRepresentation=polyline&computeTravelTimeFor=all&travelMode=bicycle&vehicleCommercial=false
```
La respuesta de tom tom tiene un formato como se muestra a configuración :
```javascript
{
   ...
   "routes": [
       {
           "summary": {
               "lengthInMeters": 3265,
               "travelTimeInSeconds": 621,
               "trafficDelayInSeconds": 0,
               "trafficLengthInMeters": 0,
               "departureTime": "2021-12-18T20:34:38-03:00",
               "arrivalTime": "2021-12-18T20:44:59-03:00",
               "noTrafficTravelTimeInSeconds": 621,
               "historicTrafficTravelTimeInSeconds": 621,
               "liveTrafficIncidentsTravelTimeInSeconds": 621
           },
           "legs": [
               {
                   "summary": {
...
Donde se puede obtener el output en formato polyline conveniente para trabajar con shapes.
